...
function scrub() {
  ...
  // --- FREE VERSION LIMIT LOGIC ---
  const initialLines = raw.split('\n');
  let textToProcess;
  if (initialLines.length > 5) {
      alert("Free Demo Limit: Only the first 5 lines were processed. Purchase Ghost Scrubber Pro for unlimited rows!");
      textToProcess = initialLines.slice(0, 5).join('\n');
  } else {
      textToProcess = raw;
  }
  // --------------------------------
